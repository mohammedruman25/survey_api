...
@Entity
@Table(name = "SURVEY_RELEASE_MAPPING_DATA_OPEN", indexes = {@Index(name = "survey_rel_map_open_index1",  columnList="survey_id", unique = false),
        @Index(name = "survey_rel_map_open_index2", columnList="target_designation_id",     unique = false),
        @Index(name = "survey_rel_map_open_index3", columnList="target_region_data_id",     unique = false)}, 
		uniqueConstraints=@UniqueConstraint(columnNames={"survey_id", "target_designation_id","target_region_data_id"}))
@NoArgsConstructor
@Getter
@Setter
@ToString
public class SurveyReleaseMappingDataOPEN implements Serializable {
